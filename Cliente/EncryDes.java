...
public class EncryDes {
    ...
    String desKey = "0123456789abcdef"; //Llave que predefinimos para desencriptar 
    byte[] keyBytes;


    public EncryDes(){
      try {
        keyBytes = DatatypeConverter.parseHexBinary(desKey); //Se le agrega la llave
        SecretKeyFactory factory = SecretKeyFactory.getInstance("DES");
        key = factory.generateSecret(new DESKeySpec(keyBytes));
        ecipher = Cipher.getInstance("DES"); //Cifrado con algoritmo DES
        dcipher = Cipher.getInstance("DES");//Descifrado con algoritmo DES
